...
from config import DATA_DIR, KNOWLEDGE_BASE_PATH


def clean_text(text: str) -> str:
    """Limpia espacios, saltos de línea y caracteres fuera del rango imprimible."""
    if not text:
        return ""
...
def chunk_text(text: str, max_chars: int = 1500, overlap: int = 150) -> list[str]:
    """Divide texto largo en chunks con solapamiento, cortando en límites de oración."""
...
def load_paginas() -> list[dict]:
    ...
    for item in data:
        ...
        contenido = clean_text(
            item.get("texto") or item.get("contenido") or item.get("text") or item.get("content") or ""
        )
        url = item.get("url", "")
        for chunk in chunk_text(contenido):
            entries.append({"fuente": "web", "titulo": titulo, "url": url, "texto": chunk})
...
def load_youtube() -> list[dict]:
    ...
    for item in data:
        ...
        for chunk in chunk_text(contenido):
            entries.append({"fuente": "youtube", "titulo": titulo, "url": url, "texto": chunk})
...
def load_wikipedia() -> list[dict]:
    ...
    for item in data:
        ...
        for chunk in chunk_text(contenido):
            entries.append({"fuente": f"wikipedia_{idioma}", "titulo": titulo, "url": url, "texto": chunk})
...
def build_knowledge_base() -> None:
    ...
    KNOWLEDGE_BASE_PATH.parent.mkdir(exist_ok=True)
